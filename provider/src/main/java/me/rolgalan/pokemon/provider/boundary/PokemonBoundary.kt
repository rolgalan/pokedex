...
public class PokemonBoundary(listener: DataInterface<Pokemon>?) : DataBoundary<Pokemon, PokemonResponse>(listener) {
    ...
    private fun mapPokemon(response: PokemonResponse): Pokemon {
    ...
    private fun mapSprites(sprites: me.rolgalan.pokemon.server.model.PokemonSprites): PokemonSprites {
        return PokemonSprites(sprites.backDefault, sprites.frontDefault)
    }

    private fun mapTypes(types: List<me.rolgalan.pokemon.server.model.PokemonType>): List<PokemonType> {
        return types.map { value -> mapType(value) }
    }

    private fun mapType(type: me.rolgalan.pokemon.server.model.PokemonType): PokemonType {
        return PokemonType(type.slot, mapNamedApiResource(type.type));
    }

    private fun mapNamedApiResource(type: me.rolgalan.pokemon.server.model.NamedApiResource): NamedApiResource {
